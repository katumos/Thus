...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:include href="SectionTOC.xsl" />
  ...
  <xsl:comment>
  Generated by SectionLayout.xsl [THUS XML System - Release 1.3.0] (c) 2006 SBSA
  </xsl:comment> 
    ...
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" >
      ...
      <xsl:apply-templates select="//my:Chapters" />
          ...
          <fo:table-row height="1.6cm" display-align="after" >
            <fo:table-cell>
              <fo:block text-align="start" xsl:use-attribute-sets="PageHeaderFont" >
                ...
                <xsl:text> | </xsl:text>
                <xsl:apply-templates select="/my:Document/my:Handbook" mode="SECTION" />
  ...
  <xsl:template match="my:Handbook" mode="TYPE" >
    <xsl:choose>
      ...
      <xsl:otherwise>
        <fo:inline color="red">handbook type</fo:inline>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  
  <xsl:template match="my:Handbook" mode="SECTION" >
    <xsl:choose>
      <xsl:when test="string-length( my:Section/my:SectionTitle ) = 0">
        <fo:inline color="red">section title</fo:inline>
      </xsl:when>
      <xsl:otherwise>
        <xsl:call-template name="std:ToLower" >
          <xsl:with-param name="text" select="my:Section/my:SectionTitle" />
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="my:Handbook" mode="YEAR" >
    <xsl:choose>
      <xsl:when test="string-length( @year ) = 0">
        <fo:inline color="red">year</fo:inline>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="my:Chapters" >
    <xsl:if test="count( child::*[string-length( normalize-space( text() ) ) &gt; 0 ] )" >
    <!-- Bookmark -->
    <fox:outline>
      ...
      <fox:label>
        <xsl:value-of select="/my:Document/my:Handbook/my:Section/my:SectionNo" />
        <xsl:text> </xsl:text>
        <xsl:value-of select="normalize-space( /my:Document/my:Handbook/my:Section/my:SectionTitle )" />
      </fox:label>
    </fox:outline>
    </xsl:if>
    <!--
    ...
    -->
    <xsl:call-template name="SectionTOC" />
