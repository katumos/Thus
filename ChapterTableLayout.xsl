...
<xsl:stylesheet version="1.0" 
  ...
  <!--
   ...
   Renders XHTML tables as XSL-FO and other stuff as well

  -->
  
  <xsl:template match="my:HtmlTableData" >
    <xsl:comment>
      Generated by TableLayout.xsl [THUS XML System - Release 1.2.0]
    </xsl:comment> 
    <fo:block>
      <xsl:apply-templates select="html:div/html:table" />
    </fo:block>
  </xsl:template>
  ...
  <xsl:attribute-set name="th">
    ...
    <xsl:attribute name="text-align">center</xsl:attribute>
    <xsl:attribute name="border">solid 1px</xsl:attribute>
  ...
  <xsl:template name="process-style">
    ...
    <xsl:if test="$name">
      ...
      <xsl:choose>
        ...
        <xsl:when test="$name = 'WIDTH' and (self::html:col or self::html:colgroup)">
          <xsl:choose>
            <xsl:when test="contains($name, 'px')">
              <xsl:attribute name="column-width"><xsl:value-of select="floor($name div 72)"/>in</xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:when test="$name = 'MIN-HEIGHT' and self::html:tr">
          <xsl:choose>
            <xsl:when test="contains($value, 'px')">
              <xsl:attribute name="height"><xsl:value-of select="substring-before($value, 'px') div 72"/>in</xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:when test="$name = 'BORDER-RIGHT' and ( self::html:th or self::html:td ) and not( contains($value, 'solid') )">
          <xsl:attribute name="border-right">none</xsl:attribute>
        </xsl:when>
        <xsl:when test="$name = 'BORDER-TOP' and ( self::html:th or self::html:td ) and not( contains($value, 'solid') )">
          <xsl:attribute name="border-top">none</xsl:attribute>
        </xsl:when>
        <xsl:when test="$name = 'BORDER-LEFT' and ( self::html:th or self::html:td ) and not( contains($value, 'solid') )">
          <xsl:attribute name="border-left">none</xsl:attribute>
        </xsl:when>
        <xsl:when test="$name = 'BORDER-BOTTOM' and ( self::html:th or self::html:td ) and not( contains($value, 'solid') )">
  ...
  <xsl:template match="html:table">
    <fo:table xsl:use-attribute-sets="table" inline-progression-dimension.minimum="100%">
  ...
  <xsl:template match="html:font/text()" >
    <xsl:choose>
      <xsl:when test="ancestor::*[@face][1]/@face = 'Symbol'" >
        <fo:inline font-family="MSSymbol" >
          <xsl:if test="ancestor::node()/@size" >
            <xsl:variable name="size" select="( ancestor::node()/@size )[1]" />
            <xsl:attribute name="font-size" >
              <xsl:choose>
                <xsl:when test="$size = '1'">8pt</xsl:when>
                <xsl:when test="$size = '2'">10pt</xsl:when>
                <xsl:when test="$size = '3'">12pt</xsl:when>
                <xsl:when test="$size = '4'">14pt</xsl:when>
                <xsl:when test="$size = '5'">18pt</xsl:when>
                <xsl:when test="$size = '6'">24pt</xsl:when>
                <xsl:when test="$size = '7'">36pt</xsl:when>
