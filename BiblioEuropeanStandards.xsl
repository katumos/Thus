...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
  ...
  <xsl:template match="my:BG_EuropeanStandards" >
    <fo:table table-layout="fixed" inline-progression-dimension.minimum="100%" break-before="page" table-omit-header-at-break="false">
        ...
        <fo:table-row>
          <fo:table-cell number-columns-spanned="5">
            <fo:block xsl:use-attribute-sets="CaptionFont" space-after="11pt">
              <xsl:choose>
                <xsl:when test="string-length( normalize-space( my:BG_EuropeanStandardsCaption ) ) = 0">
                  <fo:inline color="red">Missing Caption.</fo:inline>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="my:BG_EuropeanStandardsCaption" />
                </xsl:otherwise>
              </xsl:choose>
            </fo:block>
          </fo:table-cell>
      ...
      <fo:table-body>
        <xsl:apply-templates select="my:BG_EuropeanStandard" />
      </fo:table-body>
    </fo:table>
    
    <xsl:apply-templates select="my:BG_EuropeanStandardNote" />
  </xsl:template>
  
  <xsl:template match="my:BG_EuropeanStandard" >
  ...
  <xsl:template match="my:BG_EuropeanStandardNote" >
